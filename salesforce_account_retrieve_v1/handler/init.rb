...
class SalesforceAccountRetrieveV1
  ...
  # Retrieves a Salesforce Account record with the given parameters.
  #
  # This is a required method that is automatically called by the Kinetic Task
  # Engine.
  #
  ...
  def execute()
    ...
    uri = URI.parse("https://#{@instance}.salesforce.com/services/data/v20.0/sobjects/Account/#{@parameters['account_id']}")
    ...
    <<-RESULTS
    <results>
      <result name="Account Name">#{result['Name']}</result>
      <result name="Account Site">#{result['Site']}</result>
      <result name="Type">#{result['Type']}</result>
      <result name="Industry">#{result['Industry']}</result>
      <result name="Annual Revenue">#{result['AnnualRevenue']}</result>
      <result name="Rating">#{result['Rating']}</result>
      <result name="Phone">#{result['Phone']}</result>
      <result name="Fax">#{result['Fax']}</result>
      <result name="Website">#{result['Website']}</result>
      <result name="Ownership">#{result['Ownership']}</result>
    </results>
    RESULTS
  end
