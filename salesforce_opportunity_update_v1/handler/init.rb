...
class SalesforceOpportunityUpdateV1
  ...
  # * @fields_values - A Hash that contains the data that will be written to the
  #   Salesforce record.  The Hash is mapping Salesforce field names to the
  #   field values.
  #
  ...
  def initialize(input)
    ...
    # Store the fields from the node.xml in a Hash attribute named @field_values
    @field_values = {}
  ...
  def execute()
    ...
    # Delete the "IsPrivate" value from the @field_values Hash if it is nil, because
    # this will result in an error.
    @field_values.delete("IsPrivate") if @field_values["IsPrivate"].nil?

    # Build the JSON String that will be included in the REST API request.  We use
    # the JSON.generate() method to convert the @field_values instance variable to
    # a String of data.
    data = JSON.generate(@field_values)

    # Build and send the REST API request.  The @instance instance variable is used
    # to construct the URL.  The request is also sent as PATCH and the JSON String
    # and the headers Hash are attached to the request.
    uri = URI.parse("https://#{@instance}.salesforce.com/services/data/v20.0/sobjects/Opportunity/#{@parameters['opportunity_id']}")
    http = Net::HTTP.new(uri.host, uri.port)
    http.use_ssl = true
    http.verify_mode = OpenSSL::SSL::VERIFY_PEER
    http.ca_file = File.join(File.dirname(__FILE__), 'resources', 'cacert.pem')
    response = http.send_request('PATCH', uri.path, data, headers)

    # If the operation was successful the response body will be nil and this handler
    # will return.  Otherwise parse the response body for the error message that was
    # returned.
  ...
  #
  # ==== Parameters
  # * username (String) - The username of the Salesforce account that this
  #   handler will use for authentication.
  # * password (String) - The password of the Salesforce account that this
  #   handler will use for authentication.
  # * token (String) - The security token of the Salesforce account that this
  #   handler will use for authentication.
...
<env:Envelope xmlns:xsd="http://www.w3.org/2001/XMLSchema"
  ...
  <env:Body>
    ...
    if server_url.include?(".my.")
      instance = server_url.gsub(/.*:\/\/(.*?)\..*/, '\1') + '.my'
    else
