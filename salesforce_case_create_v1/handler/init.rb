...
class SalesforceCaseCreateV1
  ...
  def initialize(input)
    ...
    # Store the fields from the node.xml in a Hash attribute named @parameters
    @parameters = {}
  ...
  def execute
    ...
    # the JSON.generate() method to convert the @parameters instance variable to
    # a String of data.
    data = JSON.generate(@parameters)
    ...
    uri = URI.parse("https://#{@instance}.salesforce.com/services/data/v20.0/sobjects/Case")
    ...
    <<-RESULTS
    <results>
      <result name="caseid">#{result['id']}</result>
    </results>
    RESULTS
  end
